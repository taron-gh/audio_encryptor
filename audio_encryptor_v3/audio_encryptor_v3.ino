...
bool lastWasSignal = true;
int inputCountdown = 0;
const byte signalTimeBuffSize = 32;
int signalTimeBuff[signalTimeBuffSize];
byte timeBuffIndex = 0;
long timeBuffSum = 0;
  ...
  if (MODE == true) {
    ...
    if (syncStarted) {
      if (buff[0] < buff[buffSize - 1] && index == buffSize - 1 && !wasRising) {
        timeBuffIndex = timeBuffIndex < signalTimeBuffSize - 1 ? timeBuffIndex + 1 : 0;
        //        signalTime = micros() - frequencyTriggerTimer;
        signalTimeBuff[timeBuffIndex] =  micros() - frequencyTriggerTimer;
        timeBuffSum += signalTimeBuff[timeBuffIndex];
        //
        wasRising = true;
        if (timeBuffIndex == signalTimeBuffSize - 1) {
          signalTime = timeBuffSum >> 5;
          ...
          if (signalTime > 2400 && signalTime < 2800) {
            syncInputFrequencyCounter++;
            //                    Serial.println(syncInputFrequencyCounter);
          } else {
            syncInputErrors++;
            if (syncInputErrors > 50) {
              //            Serial.println(syncInputFrequencyCounter);
