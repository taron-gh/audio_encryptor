...
//input
bool inputCountingAllowed = false;
bool inputWasInRange = false;
byte maxPCSignal = 0;
byte minPCSignal = 255;
byte signalLevelBuff[256];
byte signalBuffIndex = 0;
byte buffAvarage;
byte buffMax = 0;
byte buffMin = 255;
unsigned long buffSum = 0;
unsigned long buffTimer = 0;
unsigned long syncInputFrequencyTimer = 0;
int syncInputFrequencyCounter = 0;
int syncInputErrors = 0;
byte inputCountdown = 0;
bool wasSignal = false;
bool lastWasSignal = false;

//output
bool outputCountdownAllowed = false;
bool isOutputCountdown = false;
...
void loop() {
  if (MODE == true) {
    ...
    //    buffTimer = micros();
    signalLevelBuff[signalBuffIndex] = pcSignal;
    signalBuffIndex = signalBuffIndex < 255 ? signalBuffIndex + 1 : 0;
    //    Serial.println(pcSignal);


    //Synchronization
    if (syncStarted) {
      if (signalBuffIndex != 0) {
        //        buffSum += signalLevelBuff[signalBuffIndex];
        if (signalLevelBuff[signalBuffIndex] > buffMax) buffMax = signalLevelBuff[signalBuffIndex];
        if (signalLevelBuff[signalBuffIndex] < buffMin) buffMin = signalLevelBuff[signalBuffIndex];
        if (signalBuffIndex % 3 == 0) {
          buffMax--;
          buffMin++;
        }
        ...
        //        buffSum += signalLevelBuff[0];
        //        buffAvarage = buffSum >> 7;
        maxPCSignal = buffMax;
        minPCSignal = buffMin;
//        Serial.print(buffMin);
//        Serial.print("  ");
//        Serial.println(buffMax);
        buffMax = 0;
        buffMin = 255;
      }
      //      Serial.print(minPCSignal);
      //      Serial.print("  ");
      //      Serial.println((maxPCSignal + minPCSignal)>>1);
      //      Serial.println((minPCSignal + minPCSignal)>>1);
      //      if (pcSignal <= buffAvarage + 5 && pcSignal >= buffAvarage - 5 && !inputWasInRange) {
      if (pcSignal <= ((minPCSignal + minPCSignal) >> 1) + 3 && pcSignal >= ((minPCSignal + minPCSignal) >> 1) - 3 && !inputWasInRange) {
//        Serial.print((minPCSignal + minPCSignal) >> 1);
//        Serial.print("  ");
//        Serial.print(micros() - syncInputFrequencyTimer);
//        Serial.print("  ");
//        Serial.println(syncInputFrequencyCounter);
        if (micros() - syncInputFrequencyTimer > 1000 && micros() - syncInputFrequencyTimer < 1200) {
//          Serial.println(micros() - syncInputFrequencyTimer);
          syncInputFrequencyCounter++;
//          Serial.println("a");
        } else {
          syncInputErrors++;
          if (syncInputErrors > 15) {
            Serial.println(syncInputFrequencyCounter);
            syncInputFrequencyCounter = 0;
            syncInputErrors = 0;
          }
        }
        if (syncInputFrequencyCounter > 500) {
          outputCountdownAllowed = true;
          Serial.println("a");
          if (wasSignal != lastWasSignal && inputCountdown < 11) {
            inputCountdown++;
            lastWasSignal = wasSignal;
            wasSignal = !wasSignal;
          }
          syncInputFrequencyCounter = 0;
        }
        if (inputCountdown >= 11) {
          syncStarted = false;
          sendingAllowed = true;
        }
        inputWasInRange = true;
        syncInputFrequencyTimer = micros();
      } else {
        inputWasInRange = false;
      }
      if (micros() - outputSyncSignalTimer > 996 && !outputCountdownAllowed) {
        setPin(SS_PC, LOW);
        SPI.transfer(outputSyncSignalBool ? 255 : 0);
        setPin(SS_PC, HIGH);
        outputSyncSignalTimer = micros();
        outputSyncSignalBool = !outputSyncSignalBool;
      } else if (outputCountdownAllowed && outputCountdown < 11) {
        if (millis() - outputCountdownTimer > 1000) {
          isOutputCountdown = !isOutputCountdown;
          outputCountdown++;
          outputCountdownTimer = millis();
        }
        if (isOutputCountdown && micros() - outputSyncSignalTimer > 996) {
          setPin(SS_PC, LOW);
          SPI.transfer(outputSyncSignalBool ? 255 : 0);
          setPin(SS_PC, HIGH);
          outputSyncSignalTimer = micros();
      ...
      } else if (outputCountdownAllowed) {
